// js/systems/playerRaid.js - Player-initiated raid system for attacking enemy bases
export class PlayerRaidSystem {
    ...
    calculateRaidLoot(target, successChance) {
        ...
        if (target.drugs && typeof target.drugs === 'object') {
            ...
            // Cap total drugs to 60 units
            const totalDrugs = Object.values(drugs).reduce((a, b) => a + b, 0);
            if (totalDrugs > 60) {
                const scale = 60 / totalDrugs;
                let scaledDrugs = {};
                let runningTotal = 0;
                // First, scale and floor each drug
                Object.keys(drugs).forEach(drug => {
                    scaledDrugs[drug] = Math.floor(drugs[drug] * scale);
                    runningTotal += scaledDrugs[drug];
                });
                // If we floored too much, add back units to highest drugs until we hit 60
                if (runningTotal < 60) {
                    // Sort drugs by leftover fraction descending
                    const leftovers = Object.keys(drugs).map(drug => ({
                        drug,
                        leftover: (drugs[drug] * scale) - Math.floor(drugs[drug] * scale)
                    })).sort((a, b) => b.leftover - a.leftover);
                    let i = 0;
                    while (runningTotal < 60 && i < leftovers.length) {
                        scaledDrugs[leftovers[i].drug] += 1;
                        runningTotal += 1;
                        i++;
                    }
                }
                // Replace drugs with scaledDrugs
                Object.assign(drugs, scaledDrugs);
            }
        }
